...
invCont.buildByClassificationId = async function (req, res, next) {
  ...
  if (!data || data.length === 0) {
    ...
    if (!className) {
      next({
        status: 404,
        message: "Sorry, that classification was not found.",
      });
      return;
    }
...
invCont.buildDetail = async function (req, res) {
  ...
  // NOTE: Assuming your getInventoryById model function returns an array, we get the first item
  const vehicleData = await invModel.getInventoryById(invId); 
  const vehicle = vehicleData[0] || null; // Safely get the vehicle object
  ...
  // Fetch reviews (no change here)
  const reviews = await reviewModel.getReviewsByInvId(invId);

  // **** START: NEW LOGIC FOR REVIEW DISPLAY ****
  // 1. Build the list of reviews using the utility function
  const reviewListHtml = await utilities.buildReviewList(reviews); // We added this utility in the last response
  // **** END: NEW LOGIC ****
...
invCont.buildEditView = async function (req, res, next) {
  ...
  res.render("inventory/edit", {
    // Renamed view to 'edit' assuming this is the correct EJS file
    title: "Edit " + itemName,
    nav,
